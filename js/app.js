 ...
 */
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".menu__icon-wrap")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".menu__icon-wrap")) {
                    bodyLockToggle();
        ...
        function rangeInit() {
            const priceSlider = document.querySelector("._sum");
            if (priceSlider) {
                ...
                priceSlider.noUiSlider.on("update", (function(values) {
                    const sumCount = document.querySelector("._sum-count");
                    sumCount.value = Number(values[0]);
                }));
            }
            const priceSlider2 = document.querySelector("._time");
            if (priceSlider2) {
                ...
                priceSlider2.noUiSlider.on("update", (function(values) {
                    const inputTime = document.querySelector("._time-count");
                    const timeCount = document.querySelector("._input-time");
                    inputTime.value = Number(values[0]);
                    let declination = "лет";
                    if (Number(values[0]) == 1 || Number(values[0]) == 21 || Number(values[0]) == 31) declination = "год"; else if (Number(values[0]) == 2 || Number(values[0]) == 3 || Number(values[0]) == 4 || Number(values[0]) == 22 || Number(values[0]) == 23 || Number(values[0]) == 24 || Number(values[0]) == 34) declination = "года";
                    timeCount.textContent = declination;
                    inputTime.value = Number(values[0]);
                }));
            }
            const priceStart = document.querySelector("._sum-count");
            if (priceStart) priceStart.addEventListener("change", setPriceValues);
            function setPriceValues() {
                let priceStartValue;
                if (priceStart.value != "") priceStartValue = priceStart.value;
                priceSlider.noUiSlider.set([ priceStartValue ]);
            }
            const timeStart = document.querySelector("._time-count");
            if (timeStart) timeStart.addEventListener("change", setTimeValues);
            function setTimeValues() {
                let timeStartValue;
                if (timeStart.value != "") timeStartValue = timeStart.value;
                priceSlider2.noUiSlider.set([ timeStartValue ]);
            }
        }
        rangeInit();
